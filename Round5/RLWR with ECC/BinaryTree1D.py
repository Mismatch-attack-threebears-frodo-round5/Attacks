...
# changeable
class Node:
    def __init__(self, id, left, right, value, alpha):
        self.id = id
        self.left = left
        self.right = right
        self.value = value
...
# changeable
def Atojson(node):
    return ([node.id, node.left.id, node.right.id, node.value, node.alpha] if node.left else [
        node.id, None, None, node.value, node.alpha])
...
# changeable
def newnode(left, right, value, alpha):
    global length
    length = length + 1
    node = Node(length, left, right, value, alpha)
    return (node)

# changeable
def newnodefromarray(node):
    result = newnode(None, None, node[3], node[4])
    return (result)
