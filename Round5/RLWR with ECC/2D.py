...
def RecoverCoefficient(root, S, version, index):
    ...
    while root.left != None:
        U = sp.poly(root.alpha * x ** (n+1-index) + root.beta * x ** (n-index), x)
        v[0] = root.value
        # due to ECC:
        for i in range(1,6):
            v[i] = int(t/b)
...
def Recover(root, S, version):
    ...
    for index in range(0,n,2):
        a, b = RecoverCoefficient(root, S, version, index)
...
correct = 0
q, p, t, b, n, h, mu = getParameters(version)
root = reconstruct(readjson('data\code_RLWR_1_2D.txt'))
...
print()
print()
print('RLWR with ECC, NIST level 1, dimension of the attack is 2')
...
correct = 0
q, p, t, b, n, h, mu = getParameters(version)
root = reconstruct(readjson('data\code_RLWR_3_2D.txt'))
for i in range(total):
    S = GenerateSecretPolynomial(version=version)
    recovered = Recover(root, S, version=version)
    check = 0
...
print()
print()
print('RLWR with ECC, NIST level 3, dimension of the attack is 1')
print('Average number of queries to recover one secret coefficient: ' + str(queries / (total * n)))
print('Success probability: ' + str(100 * correct / total))
print('-----------------------------------------------------------------------------')
...
correct = 0
q, p, t, b, n, h, mu = getParameters(version)
root = reconstruct(readjson('data\code_RLWR_5_2D.txt'))
for i in range(total):
    S = GenerateSecretPolynomial(version=version)
    recovered = Recover(root, S, version=version)
    check = 0
...
print()
print()
print('RLWR with ECC, NIST level 5, dimension of the attack is 1')
print('Average number of queries to recover one secret coefficient: ' + str(queries / (total * n)))
print('Success probability: ' + str(100 * correct / total))
print('-----------------------------------------------------------------------------')
