...
def RecoverCoefficient(root, S, version, index):
    ...
    while root.left != None:
        U = sp.poly(root.alpha * x ** (n+1-index), x)
        v[0] = root.value
        # due to ECC:
        for i in range(1,6):
            v[i] = int(t/b)
...
def Recover(root, S, version):
    ...
    for index in range(n):
        a = RecoverCoefficient(root, S, version, index)
...
print()
print()
print('RLWR with ECC, NIST level 1, dimension of the attack is 1')
...
correct = 0
q, p, t, b, n, h, mu = getParameters(version)
root = reconstruct(readjson('data\code_RLWR_3_1D.txt'))
for i in range(total):
    S = GenerateSecretPolynomial(version=version)
    recovered = Recover(root, S, version=version)
    check = 0
...
correct = 0
q, p, t, b, n, h, mu = getParameters(version)
root = reconstruct(readjson('data\code_RLWR_5_1D.txt'))
for i in range(total):
    S = GenerateSecretPolynomial(version=version)
    recovered = Recover(root, S, version=version)
    check = 0
