...
class Node:
    def __init__(self, id, left, right, value, alpha, beta, gamma):
        self.id = id
        self.left = left
        self.right = right
        self.value = value
...
def Atojson(node):
    return ([node.id, node.left.id, node.right.id, node.value, node.alpha, node.beta, node.gamma] if node.left else [
        node.id, None, None, node.value, node.alpha, node.beta, node.gamma])
...
def newnode(left, right, value, alpha, beta, gamma):
    global length
    length = length + 1
    node = Node(length, left, right, value, alpha, beta, gamma)
    return (node)

def newnodefromarray(node):
    result = newnode(None, None, node[3], node[4], node[5], node[6])
    return (result)
