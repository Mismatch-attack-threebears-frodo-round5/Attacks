...
root = reconstruct(readjson("data\Frodo640_2D.txt"))
total = 100
correct = 0
queries = 0
version = 640
for k in range(total):
    ...
    for i in range(0,640,2):
        for j in range(8):
            recover[i][j], recover[i+1][j] = RecoverCoefficient(root, sk, i, j, 640)
    if recover == sk:
        correct += 1
print('-----------------------------------------------------------------------')
print('Targeting Frodo640, dimension of the attack is 2:')
print()
...
root = reconstruct(readjson("data\Frodo976_2D.txt"))
total = 100
correct = 0
queries = 0
version = 976
for k in range(total):
    ...
    for i in range(version):
        for j in range(8):
            sk[i][j] = Sample(version)
    recover = version*[8*[0]]
    for i in range(0,version,2):
        for j in range(8):
            recover[i][j], recover[i+1][j] = RecoverCoefficient(root, sk, i, j, version)
    if recover == sk:
        correct += 1
print('-----------------------------------------------------------------------')
print('Targeting Frodo976, dimension of the attack is 2:')
print()
print('Average success probability: ' + str(correct/total))
...
root = reconstruct(readjson("data\Frodo1344_2D.txt"))
total = 100
correct = 0
queries = 0
version = 1344
for k in range(total):
    ...
    if recover == sk:
        correct += 1
print('Targeting Frodo1344, dimension of the attack is 2:')
print()
print('Average success probability: ' + str(correct/total))
